...
- **The glasses** (`GuideGlasses.astro`, placed on the hero chart) are the concept, so keep them. As in the original, they drop onto the eye chart, which stays foggy except through the two lenses, and drift over it like an optician's hand (the original's path); with a mouse they follow the pointer and drift again once it rests. The "Glasses on" switch under the chart turns the fog and the glasses off (the flat view). Then they travel with the reader: on scroll they glide (a spring, with a little tilt) to the headline nearest the reading line and land on its `data-focus` word, which comes into focus as they arrive, and fly home to the chart at the top. The spots are the italic word in `SectionHead`, the Practice headline, and last the footer wordmark (butter rims there). Anything on screen for 0.9s comes into focus anyway. Without script or with reduced motion there is no fog and no switch: they rest on a sharp chart and every headline is sharp. They land on headlines only, never on body text or buttons; to add a spot, put `data-focus` on a headline word.
- **Keep the glasses cheap.** They move by transforms only, never `left`/`top` (that counted as layout shift and cost 6 Lighthouse points). The sharp view is a window the size of the frame holding a copy of the chart moved back the other way, not a moving mask, so nothing is redrawn. The idle drift is planned a few seconds ahead and played as Web Animations that the browser runs off the page's thread: one at a time per element, and no `translate`/`rotate` on the SVG, or Chrome runs them on the main thread. Only pointer-follow and travel run frame by frame.
- **Motion:** the original's effects are on: the opening focus pull (CSS only, in `Hero.astro`; it replaced the opening curtain, which read as a loading screen and delayed the page by about a second), the fog and the drifting glasses, the paper grain, the reading-depth ruler, the side rail (from 1640px, where it clears the text), reveals that play once as each block comes into view (letters and fee cards staggered, the process step printing out; the script is in `Base.astro`, and anything already on screen shows at once), and two ambient videos. No custom cursor: it competed with the glasses. Videos are AI-generated clips (Gemini, made for this site: a phone on a small-town street in the hero, hands at a laptop in The practice), cut to 9-second muted loops with a crossfaded seam in `public/video/` (under ~300 KB each, with a WebP poster); `AmbientVideo.astro` plays them only while on screen and only after the page has loaded. No faces, and blur any on-screen text so AI gibberish never shows. Never hotlink stock video, and never put a clip where it can't be seen (two faint background clips were cut for that). Under `prefers-reduced-motion` everything stops: no fog, no drift or travel, no fade-ins, posters instead of video, sharp chart. With Save-Data on, no video.
