...
- **Type:** Archivo (variable width) for the heavy wide eye-chart letters (`.optotype`) and body; Fraunces for headlines, with the italic word in SOFT 80 / WONK 1. The roman loads the @fontsource `standard` file. The italic is `src/assets/fonts/fraunces-italic.woff2`: the 150 KB `full` file trimmed to those settings and weights 400–500, optical size kept (64 KB, same look; the command to regenerate it is in `global.css`). DM Mono for small `.label` text, never below 11px, and `.btn-label` (13px) inside buttons. Everything else is self-hosted via @fontsource. Helper classes (`.label`, `.btn-label`, `.optotype`, `.paper`…) sit in `@layer components`, so a Tailwind utility on the same element wins.
