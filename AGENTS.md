...
- **Motion:** everything outside the hero chart is sharp and visible by default; effects are added on top (the glasses, scroll-driven ruler and reveals). Everything stops under `prefers-reduced-motion`: the chart is sharp and the glasses just sit on it. No loading screens, custom cursors or background video.
